		...
		while ($fila = $resultado->fetch_assoc()) {
			...
			foreach($fila as $nombre_campo => $valor){
				...
				if(json_decode($comentarios[$contadorcolumna])->FKtabla != ""){
					...
					echo '<td externo="si" tabla="'.$tabla.'" claveexterna="'.json_decode($comentarios[$contadorcolumna])->FKclave.'" tablaexterna="'.json_decode($comentarios[$contadorcolumna])->FKtabla.'" columna="'.$nombrecolumna[$contadorcolumna].'" Identificador = "'.$Identificador.'" columnaexterna="'.json_decode($comentarios[$contadorcolumna])->FKmostrar.'">'.$valor." - ".$fila2['campo'].'</td>';
